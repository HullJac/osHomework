...
void DETACH(const char* fileName) {
  *out << "  bvfs_detach()" << endl;
  bvfs_detach();
  
  if (fileUnreadable(fileName))
    die("Cannot open file after detach. Error: ", strerror(errno));
}

void WRITE(int fd, void* buf, int numBytes) {
  *out << "  bvfs_write(fd, buf, " << numBytes << ")" << endl;
  int retVal = bvfs_write(fd, buf, numBytes);
  if (retVal != numBytes) {
    sprintf(message, "bvfs_write expected to return %d, received %d", numBytes, retVal);
    die(message);
  }
}

void READ(int fd, void* buf, int numBytes) {
  *out << "  bvfs_read(fd, buf, " << numBytes << ")" << endl;
  int retVal = bvfs_read(fd, buf, numBytes);
  if (retVal != numBytes) {
    sprintf(message, "bvfs_read expected to return %d, received %d", numBytes, retVal);
    die(message);
  }
}

int OPEN(const char* fileName, int mode) {
  string strMode[] = {"BVFS_RDONLY", "BVFS_WAPPEND", "BVFS_WTRUNC"};
  *out << "  bvfs_open(\"" << fileName << "\", " << strMode[mode] << ")" << endl;
  int fd = bvfs_open(fileName, mode);
  if (fd <= -1)
    die("bvfs_open failed to open file and returning", to_string(fd));

  return fd;
}

void CLOSE(int fd) {
  *out << "  bvfs_close(" << fd << ")" << endl;
  int retVal = bvfs_close(fd);
  if (retVal != 0)
    die("bvfs_close should return 0, received: ", to_string(retVal));
}
...
vector<void (*)()> testSuite {
  ...
  []() {
    ...
    *out << "  bvfs_open(\"DNE.txt\", BVFS_RDONLY)" << endl;
    redirectOutput();
    int fd = bvfs_open("DNE.txt", BVFS_RDONLY);
    string output = restoreOutput();
    if (fd != -1)
      die("bvfs_open failed to return -1 when RDONLY opening file that doesn't exist. Gave: ", to_string(fd));
    if (output.size() == 0)
      die("bvfs_open failure did not also print an error message");
  ...
  []() {
    *out << "[Fail on bvfs_close of file descriptor that isn't open]" << endl;
  ...
  []() {
    *out << "[Write int, read int]" << endl;
    ...
    INIT(defaultPartitionName);
    int fd = OPEN("somefile.data", BVFS_WAPPEND);
    WRITE(fd, &num1, sizeof(num1));
    CLOSE(fd);

    fd = OPEN("somefile.data", BVFS_RDONLY);
  ...
  []() {
    ...
    *out << "  bvfs_read(fd, buf, " << sizeof(rdNum2) << ")" << endl;
    redirectOutput();
    int retVal = bvfs_read(fd, &rdNum2, sizeof(rdNum2));
    string output = restoreOutput();
    if (!(retVal == 0 || retVal == -1))
      die("bvfs_read expected to return -1 or 0, received: ", to_string(retVal));
    if (output.size() == 0)
      die("bvfs_read failure did not also print an error message");
    CLOSE(fd);
  ...
  []() {
    *out << "[Write int, close/open, concat write int, close/open, verify]" << endl;
    ...
    fd = OPEN("somefile.data", BVFS_WAPPEND);
  ...
  []() {
    *out << "[Write int, close/open, trunc write int, close/open, verify]" << endl;
    ...
    fd = OPEN("somefile.data", BVFS_WTRUNC );
    WRITE(fd, &wrNum2, sizeof(wrNum2));
    CLOSE(fd);
  ...
  []() {
    *out << "[Fill a file (one write) then read it back]" << endl;
    ...
    INIT(defaultPartitionName);
    int fd = OPEN("full-file.data", BVFS_WAPPEND);
    WRITE(fd, inData, sizeof(inData));
    CLOSE(fd);

    fd = OPEN("full-file.data", BVFS_RDONLY);
  ...
  []() {
    *out << "[Check bvfs_ls functionality for no files]" << endl;
    ...
    if (output.size() == 0)
      die("bvfs_ls is not producing output");
    if (output.find("0 File") == string::npos)
      die("bvfs_ls is not correctly printing a header identifying the number of files. Execting: \"0 Files\". Received:\n",output);
  ...
  []() {
    *out << "[Check bvfs_ls functionality for 1 file]" << endl;
    int SZ = 51257;
    char inData[SZ];
    ...
    if (output.find("1 File") == string::npos)
      die("bvfs_ls is not correctly printing a header identifying the number of files. Execting: \"1 File\"");
    if (output.find("somefile.data") == string::npos)
      die("bvfs_ls is not correctly printing file names");
    if (output.find("51257") == string::npos)
      die("bvfs_ls reports an incorrect number of bytes in the file. Expecting 12345, received:\n", output);
    if (output.find("101") == string::npos)
      die("bvfs_ls reports an incorrect number of blocks in the file. Expecting 25, received:\n", output);
  ...
  []() {
    *out << "[init, detach, init, detach]" << endl;
    INIT(defaultPartitionName);
    DETACH(defaultPartitionName);
  ...
  []() {
    *out << "[init, write, detach, init, read, detach]" << endl;
    int num1 = 1234567890, num2 = 0;
  ...
  []() {
    *out << "[init, write multiple blocks, detach, init, read, detach]" << endl;
    const int SZ = 51256;
  ...
  []() {
    *out << "[write to 2 files, read from 2 files]" << endl;
    ...
    INIT(defaultPartitionName);
    int fd1 = OPEN("file1.data", BVFS_WAPPEND);
    WRITE(fd1, f1Ptr, 1); f1Ptr++;
    int fd2 = OPEN("file2.data", BVFS_WAPPEND);
    ...
    int out1Num, out2Num;
    fd2 = OPEN("file2.data", BVFS_RDONLY);
    fd1 = OPEN("file1.data", BVFS_RDONLY);
    ...
    if (output.find("2 File") == string::npos)
      die("bvfs_ls is not correctly identifying the existence of the 2 files. Execting: \"2 Files\"");
  ...
  []() {
    *out << "[write to 2 files, read from 2 files with detach/init inbetween]" << endl;
    int f1Num = 123456789, f2Num = 987654321;
    char *f1Ptr = (char*)(&f1Num);
    char *f2Ptr = (char*)(&f2Num);
  ...
  []() {
    *out << "[write to partion1, then partition2, read from p1, then from p2]" << endl;
    ...
    CLOSE(fd);
    DETACH(partition1Name);

    INIT(partition2Name);
    fd = OPEN("somefile.data", BVFS_WAPPEND);
    WRITE(fd, &inNum2, sizeof(inNum2));
    CLOSE(fd);
    DETACH(partition2Name);
...
int main(int argc, char** argv) {
  ...
  if (argc == 2) {
    ...
    if (testNum < 0 || testNum >= testSuite.size()) {
      ...
      cerr << "  - testNum should be in the range [0, " << testSuite.size()-1 << "]" << endl;
      return -1;
    }
