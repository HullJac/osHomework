...
struct Inode {
    ...
    // not have to change the padding when I compiled differently using the make file or my own stuff
    ...
    // Meaning I need to multiply by blockSize to get actual byte location
 ...
 */
int bvfs_detach() { 
    // Check if the partition was inited
    if (inited == 1) {
        // Close all the files
        closeAllFiles();
    ...
    if (inited == 1) {
        ...
        if (fileDescriptor != -1) { 
            ...
            // Open the file and delete everything in it
                ...
                // There always should be since I just gave some back, but just to be safe
                if (newBlock == -1) {
                    fprintf(stderr, "There are no more free blocks left.\n");
                    return -1;
                }
        ...
        // Check if the mode is a write and not read and if there is a free file
            ...
            if (isFree == -1) {
                fprintf(stderr, "There are no more free files left.\n");
                return -1;
            }
            ...
            // If it's write, create the file and write the stuff
                ...
                // Set the isFree to the fileDescriptor
...
int bvfs_close(int bvfs_FD) {
    ...
    if (inited == 1) {
        ...
        // Set all needed variables to their closed state
        else {
            ...
            if ((maxFileBytes - INList[index].numBytes) >= count) {
                ...
                while (tempCount != 0) {
                    ...
                    // We cannot fit everything in the current block
                        ...
                        // Check if there are blocks left
                        if (newBlock == -1) {
                            fprintf(stderr, "There are not enough free block to write that to file.\nSome data may not have been written to the file.\n");
                            return -1;
                        }
...
int bvfs_read(int bvfs_FD, void *buf, size_t count) {
    ...
    if (inited == 1) {
        ...
        if (INList[index].opened == 1) {
            // See if there is enough stuff to read from the file
...
int bvfs_unlink(const char* fileName) {
    ...
    if (inited == 1) {
        ...
        // Error that the file name was not found
            ...
            fprintf(stderr, "Filename <%s> was not found.\n", fileName);
            return -1;
        }
    }
...
void bvfs_ls() {
    ...
    // Create an array to reference files from
    int files[256];
